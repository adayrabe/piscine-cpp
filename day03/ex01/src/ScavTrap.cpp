...
void ScavTrap::takeDamage(unsigned int amount)
{
    ...
    {
        std::cout << "" << std::endl;
        std::cout << "SC4V-TP " << this->_name << " has 0 HP" << std::endl;
        this->_hitPoints = 0;
        return ;
    }
    this->_hitPoints = this->_hitPoints + this->_armorDamageReduction - amount;
    std::cout << "SC4V-TP " << this->_name << " receives " << amount <<
    " of damage, current hitpoints are  " << this->_hitPoints << std::endl;
    std::cout << "OUCH! How dare you?" << std::endl;
    return ;
}
...
void ScavTrap::beRepaired(unsigned int amount)
{
    ...
    {
        std::cout << "ScavTrap " << this->_name << " has not enough energy points! Adding energy points" << std::endl;
        this->addEnergyPoints();
        return ;
    }
    if (this->_hitPoints + amount >= this->_maxHitPoints)
    {
        std::cout << "SC4V-TP " << this->_name << " is now totally repaired." <<std::endl;
        std::cout << "" << std::endl;
    ...
    std::cout << "SC4V-TP " << this->_name << "is repairing his hit points by " << amount <<
    "! His current hitpoints amount is " << this->_hitPoints << std::endl;
    std::cout << "Woo-hoo! I've drank some healthy elixir (vodka, of course! just kidding). Now i'm ready\
    to continue the battle" << std::endl;
    return ;
}
...
void ScavTrap::challengNewcomer()
{
